...
module.exports = {
	...
	async execute(interaction) {
		if (interaction.isChatInputCommand()) {
			...
			//	if (now < expirationTime) {
			//		const expiredTimestamp = Math.round(expirationTime / 1000);
			//		return interaction.channel.send({ content: `Please wait, you are on a cooldown for \`${command.data.name}\`. You can use it again <t:${expiredTimestamp}:R>.`, ephemeral: true });
			//	}
			//}
